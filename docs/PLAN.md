...
| 1b Deployment | 🟨 In progress | `phase-1b-deploy` | 2026-09-18 | — | **Moved ahead of Phase 10 (§17 #238).** On **57.128.240.51** (OVHcloud, Ubuntu 24.04.4, PostgreSQL 16, Python 3.12.3), installed beside the running valleymade.uz and touching none of it (§17 #241). **Up and serving:** Redis installed and the cache proven to read and write through it; `graphix` role, database and virtualenv; `SECRET_KEY` and the database password generated on the server so neither was ever typed or transmitted; migrations, `seed_regions` (15 regions, 221 districts), `compilemessages`, `collectstatic` (192 files, hashed); `check --deploy` clean; gunicorn under systemd on `/run/graphix/graphix.sock`; nginx serving it by Host header with gzip and the hashed assets; the cart-pruning timer live. Every page answers correctly and **`POST /payment/click/update/` resolves with no redirect in front of it**. `backup.sh` and `restore-check.sh` both run green against the real database. **Waiting on Kamronbek:** the A record at Eskiz (graphix.uz still points at 45.138.159.4), then certbot; the superuser; the Click, Eskiz, Telegram and Maps credentials; and where the off-server backup goes |
