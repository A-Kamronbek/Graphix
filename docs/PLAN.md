...
**Plan version:** 2.16 · created 2026-09-08 · last amended 2026-09-16
**Status:** Phases 0, 1a, 2, 3, 4, 5, 6, 7, 8 and 12 complete · **Phase 8 complete** · Phase 1b unblocked (the VPS is bought)
...
13. **Do what the privacy policy says the server does** (§17 #183, #196). Run
    ...
    same thirty-day rotation the Django log already has (`LOG_RETENTION_DAYS`). If the VPS is outside
    Uzbekistan, the policy's cross-border section has to say so before launch (§19 Q31).
...
1. ✅ **Privacy policy** (`/privacy/`) — the data controller and how to contact them; what is collected
   ...
   *Built:* eleven sections. Written against the law as amended on 26.03.2026 (ZRU-1125), which
   ...
   sit on a server abroad provided the policy says so (§19 Q31). Registering with the state register of
...
2. ✅ **Terms of use** (`/terms/`) — parties and definitions; registration and phone verification;
   ...
   *Built:* seventeen sections, the public offer and the seller's details among them. The payment
   ...
   exchanged at the customer's cost both ways, a defect at ours (§17 #185).
...
8. **Legal pages.** Answer §19 Q27–Q31 first — three of them change what the pages say. Then, if the
   texts are otherwise unchanged, set the three `VERSIONS` dates in `core/legal.py` to the launch day:
   nobody has been shown the 2026-09-15 wording, so this is the one time an existing row may be edited.
...
| 27 | **What is the seller's legal form?** The registration record is in a person's name with a personal identification number, which suggests a sole trader (YaTT), but the pages say only the name until that is confirmed. If there is a certificate number, the terms should carry it | Before launch | ⏳ Open |
| 28 | **The registration record gives postal index 700000 for a Qoʻqon address.** That was Tashkent's index under the old numbering and cannot be Qoʻqon's, so the pages print the address without one. Worth correcting on the record itself, and telling us the real index | Before launch | ⏳ Open |
| 29 | **Which Uzpost service will parcels go by — Bir Qadam or an ordinary parcel?** A branch holds the first for 14 days and the second for a month, so the pages give "14 to 30 days" (§17 #186). One answer lets them give one number. Bound up with Q24 | Phase 11 | ⏳ Open |
| 30 | **Register the shop with the state register of personal-data operators** before the site takes a real customer's data. The owner's to do, not code — but the privacy policy is written on the assumption that it is done | Phase 11 | ⏳ Open |
| 31 | **Where is the VPS?** If it is outside Uzbekistan, the privacy policy's cross-border section has to name that transfer before launch; since 26.03.2026 the law no longer requires ordinary customer data to be stored inside the country (§9 Phase 8 item 1) | Phase 1b | ⏳ Open |
