...
**Plan version:** 2.13 · created 2026-09-08 · last amended 2026-09-14
**Status:** Phases 0, 1a, 2, 3, 4, 5, 6, 7 and 12 complete · **Phase 7 complete** · Phase 1b unblocked (the VPS is bought)
...
| 164 | 2026-09-14 | **A list the owner has to ask a developer to extend is not a list he owns** | `Tag.kind` and `Product.print_method` were `TextChoices`: adding a value meant a migration, which is the thing the panel exists to avoid (§9 Phase 7 items 7–8). Both are tables now — `TagKind` and `PrintMethod` — seeded by migration 0019 with exactly the values the code held, and swapped through a temporary column so no tag and no product lost what it had. `Product.fit` is deliberately NOT among them: two cuts is a decision the owner made (§17 #70), a size chart finds its products through that field, and it stays choices until he says otherwise. | `product/models.py`, migration 0019 |
...
| 25 | **`payment/templatetags/uzb_dates.py` prints Uzbek months in every language** | 2026-09-14 | `uz_date` and `uz_date_full` hold a hardcoded list of Uzbek month names and return them whatever language the page is in, so the customer’s own orders table and the order-detail page show "14 Sen 2026" to a Russian reader. They predate §3 and are used in exactly two templates. The replacement is Django’s own `date:"j E Y"`, which is what Phase 12 settled on for review dates (§17 #138) — so the fix is two template edits and deleting the file. Left out of 7a because it is storefront copy and 7a was panel work; small, and worth doing in **Phase 8**, which is the copy phase |

---
| 26 | The size-chart upload on `/boshqaruv/sozlamalar/` uses a bare file input, so it shows the browser's own English "Choose File" on an Uzbek screen. The panel's gallery already uses the pattern that avoids it — a visually-hidden input driven by a styled `<label>`. The review form has the same bare input, so this is one change in two places. | Phase 9 |
| 27 | `panel.js` says three things with `window.alert()` — a failed status save, a failed inline save, a failed reference save. The gallery on the same screen shows its refusals in an error line instead, which is the better pattern and the one the storefront uses. | Phase 9 |
| 28 | Deleting a product photograph removes the row but leaves the file on disk. Nothing reads it again, so it is storage rather than correctness, but it accumulates. | Phase 10 |
| 29 | The review form's file input is still a bare `type="file"`, so it shows the browser's own English "Choose File". The panel's chart upload and its gallery both use the styled-label pattern that avoids it; this is the last place that does not. | Phase 9 |
| 30 | `Product.fit` is still `TextChoices` — two cuts, by §17 #70. If a third is ever wanted it is a model change, a migration and a look at `resolve_size_chart`, the size-guide page and the shop's filter. | when Kamronbek asks |
| 31 | Two reference rows may share a name (their slugs differ). Harmless today; if the owner ever ends up with two tags called the same thing in the same kind, a warning on the create form would be kinder than letting him find out later. | Phase 9 |
