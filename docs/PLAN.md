...
**Plan version:** 2.15 · created 2026-09-08 · last amended 2026-09-15
...
| 30 | ~~`Product.fit` is still `TextChoices` — two cuts, by §17 #70~~ **✅ Closed 2026-09-14 — the field is gone (§17 #172).** A cut is a tag now, so a third one is a row the owner types, not a migration. | — |
...
| 26 | The "Yetkazilmoqda" tile links to orders whose status badge still reads "Yoʻlda" (`Order.Status.ON_THE_WAY`). Renaming the tile was asked for (§17 #178); renaming the *status* would change copy the customer reads on their own order page, so it was not done unasked. Should the status label follow the tile? | Phase 8 | ⬜ **Raised 2026-09-15** — a one-line label change, no migration: the stored value stays `on_the_way` |
