...
**Plan version:** 2.37 · created 2026-09-08 · last amended 2026-09-23
...
**rename the GitHub repository `ValleyMade` → `graphix`** (§18 #9, confirmed). *Amended 2026-09-23: the package half of this is already done.* §17 #242 flattened the repository before the first deploy and `vm/vm/` became **`config/`** — the risky half, the one that touches the settings module, wsgi, systemd, nginx and the venv, was paid for then and is live. What is left is the repository rename alone, which changes the clone URL and every local remote and which GitHub redirects, so nothing breaks the moment it happens. The package is not renamed again to `graphix`: `config` says what it holds, and moving it a second time would spend the same risk for a worse name.
...
### Phase 14 — The owner's changes

Five changes the owner asked for on 2026-09-21, after seeing the site live. They
run **before Phase 10** (§17 #250): they touch the home page, the checkout and
the payment methods, and writing Phase 10's tests against pages that are about
to be rebuilt would mean writing them twice.

1. **The home page leads with slide cards, managed from the panel.** A new
   `Slide` — image, link, order, on/off — built on the existing `Photograph`
   base so every slide gets WebP renditions and intrinsic dimensions for free.
   The hero goes; the two buttons it held (shop, contact) move directly under
   the slides; new arrivals, "Siz uchun", the categories row and the closing
   section all stay exactly as they are (§17 #251).
2. **Required fields are marked.** An asterisk and a legend saying what it
   means, `aria-hidden` on the asterisk itself so a screen reader announces
   "required" rather than "star".
3. **Sizes are editable from the panel**, beside the tags and the size charts,
   on the reference allowlist that already exists (§17 #149). `Variant.size` is
   `PROTECT`, so a size in use already refuses deletion with the right message.
4. **The home-delivery address splits in two** (§17 #252): with the map chosen
   the page shows only the map and the notes, and the picked address appears as
   a line the customer can tap to open a sheet holding region, district, street
   and house.
5. **Four ways to pay** — cash, Click, Payme, Octo — each a row the owner
   switches on and off from the panel, which `PaymentOption` has been built for
   since §17 #99. Click moves from `click-pkg` to `tolov` (§17 #253).

**Definition of Done:** the home page renders from panel-managed slides with an
`h1`, zero CLS and the first slide preloaded · the checkout marks its required
fields · sizes and payment methods edit from the panel · the address sheet works,
and with JavaScript off every field still renders inline · all four payment
methods appear when switched on and are refused server-side when off · the Click
webhook still answers at exactly `/payment/click/update/` · the suite is green.
...
```
0 → 1a → 2 → 3 → 4 → 5 → 6 → 12 → 7 → 8 → 9 → 1b → 14 → 10 → 11 → 13
```
...
| 1b Deployment | 🟨 In progress | `phase-1b-deploy` | 2026-09-18 | — | **graphix.uz is live over HTTPS as of 2026-09-19.** On 57.128.240.51 (OVHcloud, Ubuntu 24.04.4, PostgreSQL 16, Python 3.12.3), installed beside the existing site and touching none of it (§17 #241). HTTP/2, HSTS with preload, `X-Frame-Options: DENY`, nosniff, `Referrer-Policy: same-origin`; www and http both 301 to the canonical origin; hashed static served by nginx with gzip; sitemap and hreflang correct; **`POST /payment/click/update/` resolves with nothing in front of it**. Redis live and the cache proven through it. `SECRET_KEY` and the database password were generated on the server, so neither was ever typed or transmitted. `backup.sh` and `restore-check.sh` both green against the real database; the cart-pruning timer is on, the backup timer waits on a destination. Two defects found by deploying rather than reading: §17 #245 and #246. **Left:** the superuser, the Click/Eskiz/Telegram/Maps credentials, the off-server backup destination, the live 1 000 soʻm payment, Search Console and uptime monitoring — all Kamronbek’s. **indexing is switched off** until the catalogue is loaded (§17 #247) — `robots.txt` is `Disallow: /` and every page carries a `noindex`, verified live; Phase 11 deletes `SITE_INDEXABLE=False` from the server `.env`. Pushed to GitHub 2026-09-19 |
...
| 14 Owner changes | ✅ Done | `phase-14-owner` | 2026-09-21 | 2026-09-23 | Five changes the owner asked for after seeing the site live (§17 #250): panel-managed slide cards replacing the hero (#251), required-field marks, sizes and payment methods on the panel’s reference screens, the address split into map and sheet (#252), and four payment methods on `tolov` (#253). Runs before Phase 10 so its tests are written once, against the pages that will ship. **Item 3 done 2026-09-22:** sizes and payment methods edit from the settings screen, through the allowlist that was already there. Two defects on the way, both invisible to the screen itself (§17 #254, #255). **Item 2 done 2026-09-22:** the checkout marks the six fields it will refuse to do without, and says once what the mark means (§17 #256). **Item 1 done 2026-09-22:** the home page leads with panel-managed slide cards and the hero is gone (§17 #257—#259). **Item 4 done 2026-09-22:** with the map chosen the page carries the map and a one-line address that opens a sheet holding the fields (§17 #260). **Item 5 begun 2026-09-22:** Click moves off `click-pkg` onto `tolov` with nothing a customer sees changed, and three defects came with it (§17 #261—#263). **Item 5 done 2026-09-22:** Payme and Octo built and shipped switched off until their credentials exist (§17 #264). **All five items built. 780 tests.** **Item 5 hardened 2026-09-23:** a gateway that refuses now costs the customer a retry instead of 500ing the checkout, Boshqaruv says whether a switched-on method actually has credentials behind it, and Octo pay links follow DEBUG into test mode rather than quietly billing real cards from a laptop (§17 #266). **804 tests.** **Signed off 2026-09-23** — Kamronbek: “i like home page and checkout”. That closes the address-sheet clause, which was always going to be answered by somebody using it rather than by an assertion. **It does not close “zero CLS and the first slide preloaded”**, which §17 #236 says is measured and has not been: the dev database holds no slides, so there is no carousel to measure until the demo fixture is rebuilt — carried to Phase 10, which has the production-shaped harness anyway. Octo proven against the real gateway the same day: shop id and secret in `.env`, test mode on, a live pay URL returned. The unique key is still missing and now correctly blocks Octo in production rather than pretending to be ready (§17 #267). **808 tests.** Outstanding: the CLS measurement, the owner’s Payme keys and Octo’s unique key |
| 10 Testing | 🟨 In progress | `phase-10-tests` | 2026-09-18 | — | Branch cut and audited 2026-09-18; the work follows Phase 1b (§17 #238). **Audit:** `check --deploy` is already clean with `DEBUG=False` but nothing asserts it · **no §6 smoke matrix exists** — `SmokeTests` is still Phase 0's four · **no CSP at all**, to be built as a nonce middleware (§17 #239) · 703 files left in `vm/media/` by the suite, up from 575 the day §18 #42 was written · backlog #32, #41 and #42 still open here · **carried in from Phase 14 (2026-09-23):** the home carousel’s CLS, which needs the demo fixture rebuilt and the production-shaped harness this phase has anyway. **Resumed 2026-09-23**, branch recut from `main` after Phase 14 merged. **The §6 smoke matrix is done:** all 70 named URLs × anonymous / signed-in / staff, asserting the redirect *target* rather than the fact of a redirect, and 404 rather than 403 where an order belongs to somebody else. Coverage is asserted both ways, so a page added later fails the suite until somebody writes down what it should answer. It found §17 #268 on its first run. **815 tests.** Next: the CSP, enforcing rather than report-only (Kamronbek’s call) |
...
| 2026-09-19 | Task | 1b | v2.23: **graphix.uz is live over HTTPS.** Kamronbek moved the A record at Eskiz; certbot issued for the apex and www, and the real nginx site replaced the temporary one. Two defects, both found only by deploying: `http2 on;` does not exist in the nginx Ubuntu 24.04 ships, and the file that used it would have taken **the other site down too** at the next restart (§17 #245); and `certbot --nginx` cannot be used at all here, so the certificate is issued by webroot and the TLS configuration stays in the repository (§17 #246). Verified from outside: HSTS, HTTP/2, the canonical redirects, the sitemap, and the Click webhook resolving unprefixed. **Noticed and raised rather than decided:** the site is now public and crawlable with an empty catalogue (§19 Q37), and **valleymade.uz has lost its A record** — the old site is still served on the box and answers by Host header, but nothing resolves to it | Phase 1b — the superuser, the credentials, the backup destination, the live payment |
| 2026-09-19 | Task | 1b | v2.24: **Search engines kept out until there is something to sell, and CI earned its keep on day one.** Kamronbek chose to block crawlers until launch, so `SITE_INDEXABLE` now drives both `robots.txt` and a sitewide `noindex`, defaulting to *indexable* so the dangerous mistake is the recoverable one (§17 #247). The repository was pushed — `main` force-pushed over five old merge commits, `phase-1b-deploy` alongside it — and **CI failed on its first run**: a Phase 8 test had only ever passed because an empty untracked directory happened to exist on the machine that wrote it (§17 #248). Reproduced on Linux under Python 3.13 and PostgreSQL 16, fixed, and 692 tests green on both. 7 new tests | Phase 1b — the superuser, the credentials, the backup destination, the live payment |
| 2026-09-20 | Task | 1b | v2.25: **The exact SMS texts, extracted for Eskiz, and a guide for the steps only Kamronbek can do.** Both bodies pulled from the code verbatim and checked for what they cost to send: 56 and 54 characters, GSM-7, one SMS each. Two things came out of doing it. The apostrophe in them is ASCII rather than the site’s U+02BB, and that is right — recorded as §17 #249 so nobody “corrects” it into a UCS-2 message and a second round of moderation. And neither body goes through `gettext` (§18 #44): a Russian-speaking customer is sent Uzbek, which may be the right answer but is currently nobody’s decision, and the cheap moment to settle it is before Eskiz moderates rather than after | Phase 1b — Kamronbek’s six steps; the guide lists them |
| 2026-09-21 | Task | 14 | v2.26: **Phase 10 paused; the owner’s five changes become Phase 14 and run first** (§17 #250). Surveyed all five against the code before proposing anything, and three turned out cheaper than they sounded: the panel’s on/off switch for payment methods **already exists** (§17 #99 built `PaymentOption` with `is_active`; it was never put on a screen), sizes drop straight into the reference allowlist, and the checkout is already written to survive JavaScript being off — with a comment saying so — which is the precedent the address sheet has to follow. The library question was settled by measurement: `paytechuz` **downgrades Django 6.0.4 to 5.2.17**, `tolov` 2.2.0 does not and loads cleanly on 6.0.4, so Kamronbek’s choice was the right one (§17 #253). Also established that replacing `click-pkg` need not move the Click webhook at all, since we mount the view ourselves — which removes most of the risk in touching it. Cash goes on; how it is collected is §19 Q38 | Phase 14 — panel screens first, payments last |
| 2026-09-22 | Task | 14 | v2.27: **Phase 14 item 3 done — sizes and payment methods edit from the panel.** Both are new rows in the one allowlisted endpoint (§17 #149), and the screen was the easy half. The allowlist itself had been carrying an assumption nobody had needed to question: that a row keeps its name in a field called `name`. A size keeps it in `size`, so the not-blank guard, the duplicate guard and the name `delete_row` reports were all reading an attribute that does not exist — the panel would have accepted a second “M”, or a blank one, on the axis of every product’s price-and-stock grid (§17 #254). Then `makemessages` fuzzy-matched five of the new strings onto unrelated old ones — “Yangi oʻlcham” came back as “New password” — and the catalogue test caught it only because the run had dropped `-l uz` (§17 #255). Payment methods are read-only in `code` and cannot be deleted, both deliberately. **716 tests** | Item 2 — required-field marks on the checkout |
| 2026-09-22 | Task | 14 | v2.28: **Phase 14 item 2 done — the checkout marks what it will refuse to do without.** Six fields carry a star, and one line above the form says what a star means, in three languages — with the star passed into `blocktrans` as a placeholder rather than baked into the msgid, so Russian can put the mark where Russian puts it. The mark is a label, not a new rule (§17 #256): nothing gained a native `required` it did not already have, because `checkout.js` replaces both selects with drawers and a `required` on a control the customer can no longer see is a submit button that stops working for no visible reason. The index and the street address are each required by one delivery method and refused by the other, so `syncBranch` sets `aria-required` as it hides the half that does not apply — with the script blocked both halves show and neither is required on its own (§17 #107). The tests do not count stars: they remove each marked field from an otherwise valid order and watch the order fail to appear, so a mark cannot outlive the rule it describes. The two radio groups are not marked — recorded as §18 #45 rather than widened into. **726 tests** | Item 1 — the panel-managed slide cards on the home page |
| 2026-09-22 | Task | 14 | v2.29: **Phase 14 item 1 done — the home page leads with slide cards the owner manages himself.** A `Slide` on the `Photograph` base, its own panel screen and tab, and a carousel that is a scroll-snap track rather than a JavaScript slider — so with the script blocked it is still a row of cards a thumb can swipe, which is better than the “first slide only” fallback §17 #251 settled for (§17 #257). Kamronbek’s answers built: autoplay, dots under the slides and arrows either side, a card that spans the width with a gutter on both sizes and goes long on a laptop. Autoplay brought a pause button with it, which is not optional (§17 #259). The link box is an allowlist, and the panel’s inline editor needed telling separately — `set_field` runs no model validator, so without its own reader that one box is a way to put `javascript:` into an href on the busiest page of the site (§17 #258). **Two existing tests caught real defects:** a multi-line `{# #}` renders as visible page text and six of mine would have printed template source onto the home page, and every sprite icon has to be listed in the style guide — the two new ones were not. Both guards were written in earlier phases and both paid for themselves here. **756 tests** | Item 4 — the address split, then item 5, the payment methods |
| 2026-09-22 | Task | 14 | v2.30: **Phase 14 item 4 done — the home address is a map and a line, and the fields live in a sheet.** With “Xaritadan” chosen the page carries the map, then one row showing what the pin resolved to; tapping it opens a bottom sheet — the same `.picker-drawer` the region and district already use, not a second sheet to keep in step — holding region, district, street and the Boshqa note. The fields are **moved** into it rather than rendered twice, which is what keeps one value per name and what keeps the no-script promise: with the script blocked nothing moves and the form is the form it has always been (§17 #260, §17 #107). **Found on the way:** the geocoder writes the street into the address box by setting `.value`, which fires nothing — the two selects beside it already dispatch `change` for that exact reason. The summary would have shown the region and the district and not the street, which is the one part somebody drops a pin to get, and the saved draft would have missed it too. **762 tests** | Item 5 — the four payment methods on `tolov`, then one photographed pass over the home page and the checkout |
| 2026-09-22 | Task | 14 | v2.31: **Click runs on `tolov`; `click-pkg` is uninstalled.** Kamronbek’s call to split item 5 was the right one — the swap alone turned up three defects, and none of them would have been visible next to two new integrations. **The one that matters:** tolov checks a callback’s amount with `getattr(account, "amount", 0)`, hardcoded, where click-pkg read the field name from `CLICK_AMOUNT_FIELD`. Our `Order` has `total_price`, so the comparison would have been against **zero**, `InvalidAmount` raised, Click answered `error: -2`, and no order ever paid for — on the owner’s real money, found by reading the library rather than by it failing (§17 #262). An `Order.amount` property fixes it; `ONE_TIME_PAYMENT: False` would also silence it and must not be used, because it drops the check to “greater than zero”. Also: tolov’s app cannot be relabelled off `django` because its own second migration names that label as a dependency (§17 #261), and `payment/admin.py`’s unregister of the gateway transaction admin was running **before** the register and doing nothing — admin autodiscovery walks INSTALLED_APPS in order, and the third-party app sat last (§17 #263). `apply_successful_payment` now takes the order rather than a Click transaction id, so it no longer knows about any one gateway and stays the only place an order becomes paid. The webhook is still at exactly `/payment/click/update/`, asserted. **768 tests** | Payme and Octo, then one photographed pass over the home page and the checkout |
| 2026-09-22 | Task | 14 | v2.32: **Payme and Octo built; all five of the owner’s changes are done.** Both are `PaymentOption` rows shipped **switched off**, because there are no credentials yet — which is what these being rows was always for (§17 #99): the owner pastes his keys and flips the switch, and until then the checkout does not offer the method and refuses it server-side if one is POSTed anyway. Every new setting reads with `os.environ.get`, not `os.environ`, so a missing key cannot stop the site booting (§17 #264). `generate_paylink` is the one place that knows which client builds which link, and cash raises rather than returning something falsy — a blank redirect is how the first bug in a payment flow hides. Each gateway gets its own unprefixed callback, and a test asserts all three resolve and that none is served under a language prefix. **Two traps the library set and the reading caught:** Octo’s constructor takes `octo_shop_id` as an **int**, and the setting arrives from the environment as text that is blank until the owner has one, so `int("")` would have raised at import; and Payme’s account key must be `order_id` at both ends — it is simultaneously the key inside Payme’s `account` object and what the webhook looks the order up by, with tolov special-casing that one name to mean the model’s `id`. §18 #45 closed with it: the payment radio group carries its required mark now, because with four methods live nothing is pre-checked any more. **776 tests** | One photographed pass over the home page and the checkout, then Phase 10 |
| 2026-09-22 | Task | 14 | v2.33: **Definition of Done, read line by line rather than declared.** Five of its seven clauses are met and tested: the checkout marks its required fields; sizes and payment methods edit from the panel; every field of the address still renders inline with the script blocked; all four payment methods appear when switched on and are refused server-side when off (a test per half — rendering and accepting are different mechanisms, and a method can pass one while failing the other); the Click webhook still answers at exactly `/payment/click/update/`. **Two clauses are not met and are not being claimed:** “zero CLS and the first slide preloaded” is built for — aspect-ratio boxes, stored pixel dimensions, `fetchpriority` on the first slide — but Phase 9’s rule is that this is **measured, not asserted** (§17 #236), and it has not been measured; and “the address sheet works” covers behaviour only a browser can show. Both need the photographed pass, which rebuilds the demo fixture in the dev database and so waits for Kamronbek. **780 tests** | The visual pass, then Phase 10 |
| 2026-09-23 | Task | 14 | v2.33: **A 500 on the dev home page, and the runbook gap behind it.** Kamronbek opened `/uz/` and got `relation "product_slide" does not exist`: Phase 14 added five migrations and none had been applied to his database. Not a code fault — `migrate` fixed it, and the page now answers 200 with four payment rows (`click` on, `cash`, `payme` and `octo` off) and the no-slides branch rendering its visible `h1`. What the failure was worth is the question it raised: **would this have happened on the server too?** It would. `deploy/RUNBOOK.md` was a first-install procedure end to end, nineteen sections with no “ship an update” among them, so nothing written down said pull, install, migrate, compile, collect, restart — in that order. New §20 says it, and says the three things this particular update needs that a routine one does not: `pip install -r requirements.txt` **before** `migrate`, because migrate imports the app and the app now imports `tolov`; `pip uninstall click-pkg`, because pip removes nothing on its own and two packages both registering a Django app is not harmless; and `compilemessages`, because the `.mo` files are gitignored and skipping it renders Uzbek to a Russian visitor with no error at all (§17 #265) | The visual pass, then Phase 10 |
| 2026-09-23 | Task | 14 | v2.34: **Octo 500’d the checkout, and the reason was worse than the missing credentials.** Kamronbek switched Octo on to test it and got an `AttributeError` off tolov’s client. Octo answers an error with `"data": null` and tolov reads `response.get("data", {}).get("octo_pay_url", "")`, where the `{}` default cannot fire — so a wrong secret, a rejected amount or a bad day at Octo would all have done the same thing, with real credentials in place. The library’s bug, our hole: `generate_paylink` was the one external call on the site not following §4’s `core/sms.py` pattern, and it had got away with it because Click and Payme build their links locally and never go over the wire. Now: settings are asked before anything calls out, the call is wrapped, and “nothing configured” and “the gateway said no” are different exceptions because they are different sentences to the customer — the second one is worth retrying and the order stays PAYING for it. **Two more from the same reading:** `OctoGateway` was built without `is_test_mode`, so every pay link from a laptop carried `"test": false` and would have moved **real money**; and Boshqaruv showed the on/off switch while nothing anywhere said whether the shop could actually take money with it — a read-only badge now does, from the same function the checkout uses (§17 #266). msgmerge tried it again: “Sozlangan” came back as “Paid”. **804 tests** | The visual pass, then Phase 10 |
| 2026-09-23 | Task | 14 | v2.35: **Phase 14 closed.** Kamronbek signed off the home page and the checkout — which answers the address-sheet clause, the one that was always going to need somebody using it — and put the Octo shop id and secret in `.env`. **Octo proven against the real gateway:** test mode on, a live pay URL returned through our own `generate_paylink`, no money involved. **The verification found one more thing, in code written the same morning.** He left `OCTO_UNIQUE_KEY` blank because the owner has not been given it, and `method_is_configured` did not ask for it — so the new badge would have read **Sozlangan** on production while tolov refused to construct the Octo callback at all, which is the worst shape available: enough credentials to take a payment, not enough to confirm one. It is now a live-only credential, required exactly when tolov requires it, and test mode is read from one setting by both the pay link and the check rather than each reading DEBUG (§17 #267). Also swept: `check --deploy` clean with DEBUG off, `makemigrations --check` clean, nothing untracked, no secret tracked, and the runbook now documents all three webhooks instead of Click alone. **808 tests.** **Not closed:** “zero CLS and the first slide preloaded” is still unmeasured — the dev database holds no slides, so there is no carousel to measure until the demo fixture is rebuilt; carried to Phase 10 | Phase 10 |
| 2026-09-23 | Task | 10 | v2.36: **Phase 14 merged into `main`; Phase 10 resumed with the §6 smoke matrix.** Kamronbek chose the order: the matrix first, because it is the cheapest thing in the phase and the broadest safety net, and changing every photograph on the site for the CSP with only Phase 0’s four smoke tests watching is the wrong way round. **70 named URLs × three callers**, and three things make it worth more than 210 assertions of 200: a redirect must name where it lands, because “sent to the login page” and “sent to the home page” are both 302 and only one is the site working; an order-scoped page must answer **404** to anybody but its owner, staff included, which is a security property rather than a rendering; and coverage is asserted in both directions, so a page added in a later phase fails the suite until somebody decides what it should answer. **It found a defect on its first run** — `/boshqaruv/style/` sent a signed-in customer to `/admin/login/` where its twenty-three neighbours answer 403, because the style guide is Phase 2’s and kept Django’s `staff_member_required` when Phase 7 wrote the guard replacing it. Two existing tests covered that page and both only asked whether staff get 200 (§17 #268). Built the matrix by discovery first — printing what the site actually answers before writing down what it should — so the table encodes judgement about real behaviour rather than assumptions. **815 tests** | The CSP: three inline handlers into JS, then the nonce middleware |
...
| 250 | 2026-09-21 | **The owner's five changes become Phase 14, and it runs before Phase 10** | Kamronbek: *“for now, we stop phase 10. owner wants to change some things. and i think it is better to do them first before tests.”* He is right, and the reason is worth writing down: Phase 10's largest missing piece is a smoke matrix over every URL in §6, and two of these five changes rebuild the home page and the checkout. Tests written first would be rewritten a week later, and rewritten tests are how a suite stops being trusted. Phase 10 keeps its branch and its audit; nothing in it is discarded. Order is now `… → 9 → 1b → 14 → 10 → 11 → 13` | §13, §15, §9 Phase 14 |
| 251 | 2026-09-21 | **The home page leads with panel-managed slide cards; the hero goes** | The owner wants what every Uzbek marketplace leads with — a row of tappable promotional cards he can change himself — rather than a fixed hero. So a `Slide` model on the `Photograph` base (renditions and intrinsic dimensions for free, §17 #230) with an image, a link, an order and an on/off switch, edited from the panel. Three things carry over from what it replaces, deliberately: the height cap from §17 #29, because a full-viewport carousel on a shop is the same conversion killer a full-viewport hero was; an `h1`, visually hidden if the slides carry no heading of their own, because a page without one is an accessibility and SEO regression against a measured 100; and the no-JavaScript rule — with the script blocked the first slide simply shows. The categories row and the closing section stay (Kamronbek's answer). One quiet benefit: the “100+ dizayn” line disappears with the hero, which is what §12 risk #14 and Phase 11 item 2 were about | `product/models.py`, `templates/core/home.html`, `panel/`, `static/js/` |
| 252 | 2026-09-21 | **The home-delivery address splits: map and notes on the page, the rest in a sheet** | Modelled on what Yandex Go does, which the owner pointed at. With the map chosen the page carries only the map and the note field; the address the pin resolved to shows underneath as a line, and tapping it opens a bottom sheet — not a new page — holding region, district, street and house. **With JavaScript off every one of those fields renders inline, exactly as today.** That is not a nicety: `checkout.js` already carries a comment explaining that the three field groups deliberately have no `hidden` attribute so a blocked script leaves a working form, and a sheet that hid them behind JavaScript would quietly undo it | `templates/payment/checkout.html`, `static/js/checkout.js`, `static/css/pages.css` |
| 253 | 2026-09-21 | **`tolov` replaces `click-pkg`, and carries Payme and Octo with it** | Kamronbek's call, and the right one on the evidence. The obvious alternative, `paytechuz`, pins `django<6.0` in its `[django]` extra: installing it **downgrades Django 6.0.4 to 5.2.17**, measured, not guessed. `tolov` 2.2.0 requires `django>=3.2` with no upper bound, installs beside Django 6.0.4 untouched, and its app registry, `BaseClickWebhookView`, `BasePaymeWebhookView` and `BaseOctoWebhookView` all load on 6.0.4 in a clean venv. Two conditions on the migration, both non-negotiable: **the Click webhook stays at exactly `/payment/click/update/`** — we mount the view ourselves, so nothing is re-registered with Click — and **`apply_successful_payment` remains the only place an order becomes paid**, with tolov's success hook calling into it rather than replacing it. The cost is honest: six new packages (`httpx`, `httpcore`, `anyio`, `h11`, `loguru`, `typing_extensions`), a far larger exception to §4 than `redis` was, and tolov's Django app registers under the app label `django`, which is ugly in migration output. Cash goes live as a fourth row; how the money is collected on an Uzpost parcel is the owner's problem to solve and is recorded as §19 Q38 rather than blocking the switch | `requirements.txt`, `payment/`, `config/urls.py`, §4 |
| 254 | 2026-09-22 | **The reference allowlist is told where a row keeps its name** | Every guard in `panel/reference.py` that protects a name read a field literally called `name`: the not-blank check, the duplicate check, and the label `delete_row` hands back to the toast. Six of the eight tables have one. `Size` does not — it keeps its label in `size` — so adding it to the allowlist as it stood would have given the owner a box that accepts a second “M”, or an empty one, on the axis of every product’s price-and-stock grid, with `Variant.size` a foreign key so the damage is one row wide and every product deep. `NAME_FIELD = {'size': 'size'}` is the whole fix, read by all three. It was needed in **two** places on the rename path, not one: the duplicate check keyed on the literal `'name'`, so the first version of this change guarded creating a twin and left renaming onto one wide open — which is what a test written against the seeded S/M/L/XL found, by refusing a rename the test itself expected to succeed | `panel/reference.py`, `core.test_phase14.SizeReferenceTests` |
| 255 | 2026-09-22 | **`makemessages` guessed five Phase 14 translations, and every guess was wrong** | msgmerge fuzzy-matches a new msgid against a similar old one and copies its translation across. “Yangi oʻlcham” came back as “New password” (from “Yangi parol”), “Toʻlov usullari” as “Choose a payment method.”, “Oʻlcham qoʻshildi.” as “Tag added.”. gettext ignores a fuzzy entry, so the only symptom is Uzbek on a Russian screen — but the flag is one keystroke from being cleared by somebody who does not read the entry, and then the button says “New password”. §17 #198’s test catches the flag; nothing catches a wrong translation once the flag is gone. So each remaining Phase 14 task ends the same way: run the documented `makemessages` for **all three** languages, read every entry msgmerge touched, and assert the **rendered page** in ru and en rather than the catalogue. Dropping `-l uz` is what surfaced this at all — the uz catalogue went out of step and `test_the_catalogues_cover_the_same_strings` failed, 714 of 715 green | `locale/*/LC_MESSAGES/django.po`, `CONTRIBUTING.md`, `core.test_phase14.SettingsScreenTests` |
| 256 | 2026-09-22 | **The required mark is a label, not a new rule** | The owner asked for a star, and a star is what this adds. Giving a field a native `required` it did not have would change what the browser blocks — a behaviour change item 2 does not call for. So: a static star on the six fields the server actually refuses to do without (`Order.clean` plus the checkout view), and nothing else moved. The two selects get `aria-required` rather than `required`, because `checkout.js` replaces both with drawers and a native `required` on a control the customer can no longer see is a submit button that stops working with nothing on screen to explain it. The index and the street address are each required by one method and refused by the other, so `syncBranch` sets their `aria-required` as it hides the half that does not apply and the markup carries none — with the script blocked both halves show and neither field is required on its own (§17 #107). Every star is `aria-hidden`: the field already says it is required through the attribute, and without hiding it a listener hears the label, then “star”, then “required”. The tests are the part worth keeping — they do not count stars, they remove each marked field from a valid order and require the order to fail, so a mark and the rule behind it cannot drift apart | `templates/payment/checkout.html`, `static/js/checkout.js`, `static/css/components.css`, `boshqaruv/style.html`, `core.test_phase14.RequiredFieldMarksTests` |
| 257 | 2026-09-22 | **The home page’s carousel is a scroll-snap track, not a JavaScript slider** | §17 #251 planned for the script being blocked by showing the first slide and nothing else. A track of `scroll-snap-align` cards is better and simpler: the browser does the swiping, the momentum and the snapping, so with no script at all the owner’s cards are still a row somebody can thumb through. The script adds the arrows, the dots and the advancing and nothing else, which is why all three controls ship `hidden` and are revealed from inside it — a control that cannot do anything is worse than no control. Position is read back off `scrollLeft` rather than held in a variable, so a swipe, an arrow, a dot and a keyboard scroll cannot disagree about which slide is showing. Kamronbek’s answers: autoplay, dots under the slides and arrows either side, the card spanning the width with a gutter on both phone and laptop, and long on a laptop — two aspect ratios, 16:9 and 1120:300, which means the same upload is cropped two ways and the panel says so where the owner will read it. Two things carry over from the hero deliberately: the height cap behind §17 #29, now the ratio, and an `h1`, visually hidden while the slides carry the page and shown when there are none — the page had no other heading. **`alt` is required**, which §17 #251 did not say and the owner did not ask for: a slide is an image inside a link, and a link whose only content is an image has no accessible name at all. That is axe-core `link-name`, and Phase 9 shipped zero violations over ten pages | `product/models.py`, `templates/core/home.html`, `static/css/pages.css`, `static/js/slides.js`, `panel/`, `core.test_phase14` |
| 258 | 2026-09-22 | **A slide’s link is an allowlist, and the panel’s inline editor had to be told separately** | The owner types this into a box and the value becomes an `href` on the busiest page of the site. Two shapes are allowed — a path beginning `/`, or an `http(s)://` address — and everything else is refused, blank included as a real answer for a card that announces something without linking anywhere. `javascript:` in an href is a script injection with a human talked into performing it; `//host/path` is protocol-relative, which is an off-site link that reads like an internal one. The part worth writing down is the second door: `panel/reference.py`’s `set_field` saves with `update_fields`, which runs **no** model validator, so putting `link` in the allowlist as an ordinary `text` field would have left the model validator guarding the upload form and nothing guarding the box beside it. It has its own reader, and that reader calls the model’s validator rather than restating the rule — two copies of “what counts as a safe href” is one copy that gets relaxed later without the other noticing | `product/models.py`, `panel/reference.py`, `core.test_phase14.SlideLinkTests`, `core.test_phase14.SlideInlineEditTests` |
| 259 | 2026-09-22 | **Autoplay ships with a pause button** | Kamronbek asked for autoplay, and WCAG 2.2.2 is not a matter of taste: anything that starts moving on its own and runs for more than five seconds beside other content needs a mechanism to stop it. The carousel pauses on hover, on focus within, and while the tab is hidden, and none of those is that mechanism — they are side effects of doing something else, and they leave out anybody reading the page with neither a pointer nor a keyboard on it. So there is a real button, in the dots row, with two icons new to the sprite and both its words rendered by gettext rather than assembled in the script (§17 #80). `prefers-reduced-motion: reduce` is honoured twice over: the script never starts the timer, and the CSS drops the smooth scroll. Adding the icons cost a second fix — `core.test_backlog` asserts that every symbol in the sprite is also listed in the style guide, and it caught both | `templates/core/home.html`, `static/js/slides.js`, `templates/partials/_icons.svg.html`, `core/views.py` |
| 260 | 2026-09-22 | **The address sheet moves the fields into it; it does not render them twice** | The owner asked for what Yandex Go does: with the map chosen, the map and a note, the resolved address as a line underneath, and the rest behind a tap (§17 #252). The implementation question was whether the sheet gets its own copy of the four fields or the real ones. Its own copy is two controls named `region` inside one form — two values posted under one name, with nothing on the server able to say which the customer meant — so `checkout.js` moves the live nodes in and out instead. Three consequences worth writing down. The markup ships with the sheet empty and the summary `hidden`, so with the script blocked nothing moves and every field renders inline exactly as before (§17 #107); the sheet is `.picker-drawer`, the component the region and district drawers already open, rather than a second bottom sheet to keep in step with the first; and putting the fields back has to run last-first, because the four are adjacent siblings and a node’s “next” anchor may be another node that is also still in the sheet. The sheet is home delivery only: a branch order needs its region and district on the page, and fields left in a sheet with nothing to open it would be a form that cannot be finished | `templates/payment/checkout.html`, `static/js/checkout.js`, `static/css/components.css`, `core.test_phase14.AddressSheetTests` |
| 261 | 2026-09-22 | **tolov’s Django app keeps the app label `django`, because its own migration depends on it** | The package sets no `label` on its AppConfig, so Django derives one from the last component of `tolov.integrations.django` and the app comes out labelled **django**. That puts a table called `django_paymenttransaction` in the database and prints migrations as `django.0001_initial`, which reads like a core Django migration and is not one. On a table nobody has created yet the rename looked free, so it was tried — a one-line AppConfig subclass setting `label = "tolov"` — and it fails: the package’s second migration hardcodes `dependencies = [("django", "0001_initial")]`, so relabelling makes the graph unresolvable and `showmigrations` raises `NodeNotFoundError`. Patching a vendored migration to fix a cosmetic label is a worse trade than living with the label, so the label stays and this is the note explaining why it looks wrong | `config/settings.py` |
| 262 | 2026-09-22 | **`Order.amount` exists because tolov hardcodes the field it checks a payment against** | click-pkg took the name from `CLICK_AMOUNT_FIELD`, which this project set to `total_price`. tolov does not: `internal_webhooks/click.py` reads `getattr(account, "amount", 0)`, with a comment in the library conceding it “should be configurable”. Our `Order` has no `amount`, so the default applies and `_validate_amount` compares a real payment against **zero**; with `ONE_TIME_PAYMENT` on (the default) anything more than a cent out raises `InvalidAmount`, Click is answered `error: -2`, and **no order can ever be paid for**. Found by reading the library before trusting it, which is the only way it could have been found before a customer’s money. The fix is an `Order.amount` property returning `total_price` — an alias, not a second source of truth, and no migration. `ONE_TIME_PAYMENT: False` silences the same error and must never be used for it: it reduces the amount check to “greater than zero”, which would accept 1 000 soʻm for a 400 000 soʻm order | `payment/models.py`, `config/settings.py`, `core.test_phase14.TolovSwapTests` |
| 263 | 2026-09-22 | **A third-party app whose admin we override has to be listed before us** | `payment/admin.py` has unregistered the payment gateway’s transaction admin since Phase 4, with a comment saying raw transactions are not exposed. It was not working. Admin autodiscovery imports each app’s `admin` module in INSTALLED_APPS order, and the gateway package sat at the bottom of the list — so `payment/admin.py` unregistered a model that had not been registered yet, the `NotRegistered` was swallowed by the `try`, and the gateway app registered it seconds later. `tolov.integrations.django` now sits above our own apps, and a test asserts the model is absent from `admin.site._registry` rather than trusting the call. Worth reading twice: the code was correct, the comment was correct, the ordering was not, and nothing failed | `config/settings.py`, `payment/admin.py`, `core.test_phase14.TolovSwapTests` |
| 264 | 2026-09-22 | **A payment method with no credentials is a row that is switched off, not a setting that stops the site** | Payme and Octo arrive before their keys do. Every other required secret on this site reads with `os.environ[...]`, which is right for the ones without which nothing works; these two read with `os.environ.get(..., "")`, because a blank key here is a normal state that lasts until the owner has been through two merchant onboardings. The shape is the Maps key’s (§17 #93): blank credentials mean the `PaymentOption` row stays `is_active=False`, the checkout renders no such choice, and `create_order_from_cart` refuses the code if one is POSTed anyway — so there is no state in which a customer reaches a gateway the shop cannot talk to. `generate_paylink` raises `PaymentMethodUnavailable` rather than returning a falsy link, and the view says so in words: cash has no online step either, and both cases end with the customer being sent somewhere, which is exactly where a silent `None` would have become a blank page nobody could explain | `config/settings.py`, `payment/services.py`, `payment/views.py`, `payment/migrations/0022_seed_payme_and_octo.py`, `core.test_phase14.PaymeAndOctoTests` |
| 265 | 2026-09-23 | **The runbook grows an “update” procedure; it only ever described a first install** | A 500 on the development home page (`relation "product_slide" does not exist`) was five unapplied migrations and a one-command fix. The useful part was asking whether the server would have done the same, and it would have: sections 0 to 19 of `deploy/RUNBOOK.md` stand a machine up from nothing, and there was no section for the far more frequent case where the site is already running and has to catch up with a merge. On a developer’s machine `runserver` at least prints an unapplied-migration warning, which scrolls past; on a server nothing is printed and the first symptom is a visitor on an error page. §20 writes the order down — dump, pull, **install**, migrate, compilemessages, collectstatic, check, restart, verify — and the order is the content: `migrate` imports the application, so a new dependency has to be on disk before it runs. Three notes it carries because Phase 14 needs all three and a routine update needs none of them: `pip install` never removes a dropped package, so `click-pkg` has to be uninstalled by hand and two packages both registering a Django app is not a harmless leftover; `compilemessages` is not optional because the `.mo` files are gitignored, and skipping it makes the site monolingual with no error anywhere; and the dump comes first because an update is exactly when a migration is about to touch tables with real orders in them | `deploy/RUNBOOK.md` |
| 266 | 2026-09-23 | **A gateway that refuses must cost a retry, not a 500 — and Octo is the first one that can refuse** | Kamronbek switched Octo on to test it, and `POST /uz/payment/45/start/` answered `AttributeError: 'NoneType' object has no attribute 'get'`. Reproduced against Octo’s own API: it replies `{"error": 2, "errMessage": "0 identifikatorli doʻkon topilmadi", "data": null}`, and tolov’s client does `response.get("data", {}).get("octo_pay_url", "")` — where the `{}` default **never fires**, because the key is present and its value is null. So *every* Octo error 500s the checkout, not only the blank-credential one that found it: a wrong secret, a rejected amount, Octo degraded. The bug is tolov’s; the hole was ours. §4 has said since Phase 2 that an external call never breaks a request, and `generate_paylink` was the one external call on this site not following the `core/sms.py` pattern. It never showed because **Click and Payme build their links locally** — string concatenation and base64 — so for two phases nothing went over the wire at that moment. Octo is the first gateway that calls out to mint one. Three parts to the fix, and the split is the content: `method_is_configured` asks settings **before** anything calls out, so a switched-on method with no keys is refused without a round-trip; the gateway call is wrapped and everything caught and logged; and the two failures are different exceptions, because the customer is told different things — `PaymentMethodUnavailable` is permanent (cash, or nothing configured), `PaymentGatewayError` is “try again shortly” — with the order left PAYING through both, so it can still be paid for. An empty link counts as a failure: `redirect('')` is a bug of its own. **Two more found in the same reading.** `OctoGateway` was being constructed without `is_test_mode`, which defaults False and becomes `"test": false` in Octo’s request body — so every pay link built on a developer’s laptop was a **live payment**, while `TOLOV['OCTO_BANK']['TEST_MODE']` already read DEBUG on the callback side; the two halves now agree. And Boshqaruv showed the switch but never whether the shop could take money with it — a read-only **Sozlangan / Sozlanmagan** badge now says so, from the same function `generate_paylink` uses, because two answers to that question is exactly how a method comes to look ready in the panel and fail at the checkout. The switch stays editable: credentials added to `.env` after the last restart are a real case. `makemessages` had to be watched again (§17 #255) — it offered «Оплачен» / “Paid” for “Sozlangan”, and the rate-limit message “Too many attempts” for the gateway retry, which blames the customer for Octo’s bad day | `payment/services.py`, `payment/views.py`, `panel/views.py`, `templates/boshqaruv/settings.html`, `locale/*`, `core.test_phase14.GatewayRefusalTests`, `core.test_phase14.GatewayRefusalViewTests`, `core.test_phase14.PaymentConfiguredBadgeTests` |
| 267 | 2026-09-23 | **Octo’s unique key is a production credential, and test mode is one setting rather than DEBUG read twice** | Kamronbek put the Octo shop id and secret in `.env` and left `OCTO_UNIQUE_KEY` blank, since the owner has not been given it. That is fine for testing and **not** fine on the server, and the badge built the same morning (§17 #266) would have said so wrongly: `method_is_configured` asked for shop id and secret, both present, so Boshqaruv would have read **Sozlangan** on production while every Octo callback raised. tolov refuses to construct the Octo webhook at all without the key once test mode is off — Octo signs callbacks `sha1(unique_key + payment_uuid + status)` — so the shape of the failure is the worst one available: credentials enough to **take** a payment and not enough to **confirm** it, meaning a customer pays and the order sits unpaid forever. `LIVE_ONLY_CREDENTIALS` now names the key and `method_is_configured` requires it whenever test mode is off, which is exactly the rule tolov enforces — so Octo reads Sozlanmagan in production until the key exists, and no Octo payment can start. The second half is the one that keeps it honest: **test mode is read from `TOLOV['OCTO_BANK']['TEST_MODE']`, by both the pay link and this check**, rather than each reading `DEBUG` separately. #266 had wired `is_test_mode=settings.DEBUG` into the link while the webhook read the setting; the two agree today and nothing made them. One answer to “is this a test payment”, so a link built as a test cannot meet a callback expecting a live one, and §18 #46’s staging case becomes one line. Verified against the real gateway: with the keys in and test mode on, `generate_paylink` returns an Octo pay URL; with test mode off and no unique key, it refuses before any network call, and `OctoWebhookAPIView()` raises — asserted, so a tolov upgrade that drops the requirement tells us rather than leaving a method switched off for a reason that stopped being true | `payment/services.py`, `.env.example`, `deploy/RUNBOOK.md` §17.2, `core.test_phase14.GatewayRefusalTests` |
| 268 | 2026-09-23 | **The style guide kept the decorator the panel was written to replace** | Phase 10’s smoke matrix asks every named URL what it answers for an anonymous visitor, a signed-in customer and a member of staff, and one row came back different from its twenty-three neighbours: `/boshqaruv/style/` sent a **signed-in customer** to `/admin/login/`, where every other page under that prefix answers 403. The cause is five phases old. The style guide is Phase 2’s and used Django’s `staff_member_required`; `panel/auth.py` arrived in Phase 7 with a docstring explaining at length why that decorator is wrong here — it *“sends everyone who fails to the admin login form, so a signed-in customer who guesses the URL is shown a second login page and invited to try again”* — and the page it was describing was never moved onto it. So the site had a written rule, a working implementation of that rule, and one page quietly exempt from it: the shape §17 #263 had already produced once, where the code and the comment were both right and something outside them was not. It now uses `staff_only`. `panel.auth` imports nothing from any app, so `core` importing it adds no dependency worth avoiding — moving the route into `panel/urls.py` would have been tidier and would have risked the URL name, which never changes (§4). **Worth noting how it was found:** not by review and not by a test of the style guide, of which there were two, both asserting only that staff get 200. A matrix that asks the same question of every page finds the page that answers differently | `core/views.py`, `core/test_smoke.py` |
| 249 | 2026-09-20 | **The SMS bodies keep a plain ASCII apostrophe, against the site's own spelling rule** | The site writes Uzbek with U+02BB throughout (§4), and the two SMS bodies do not: they say `ro'yxatdan` and `o'tish` with a typewriter apostrophe. That is correct and must stay. `ʻ` is outside the GSM-7 alphabet, and one character outside it moves the whole message to UCS-2, where a part is 70 characters rather than 160 and some operators render it poorly. Both bodies are currently 56 and 54 characters and cost one SMS each. Recorded because it looks exactly like an oversight and a later typography pass would “fix” it — which would also mean asking Eskiz to moderate both messages again, since Eskiz moderates the text and not just the sender | `user/otp.py`, `user/password_reset.py` |
| 247 | 2026-09-19 | **The site is not indexable until the catalogue exists, behind one setting** | §19 Q37, Kamronbek's call. graphix.uz went live public, crawlable and empty, with a home page promising a hundred designs. `Disallow:` alone would not do it — it stops a crawler fetching a page, not the address appearing in results on the strength of a link elsewhere, with no snippet and no way to correct it — so `SITE_INDEXABLE` drives **both** `robots.txt` and a real `noindex` on every page. **The default is True**, which looks backwards today and is right for the life of the site: a `noindex` left on after launch is invisible and costs months of traffic, while an `Allow` left on before launch costs an afternoon. The pre-launch state is an explicit `SITE_INDEXABLE=False` in the server's `.env`, which Phase 11 deletes. Two things the new tests caught immediately: the flag's `noindex` started as the *default* of the `robots` block, and `shop.html` fills that block — so the shop page, the one that most needs hiding while the catalogue is empty, came out indexable; the flag now wraps the block instead. And the deployment-settings test read `settings.SECURE_SSL_REDIRECT`, which is always False under test, because those settings live inside `if not DEBUG:` evaluated at import, long before Django sets DEBUG False for the run — the file's source is what can honestly be asserted | `config/settings.py`, `core/context_processors.py`, `templates/base.html`, `templates/robots.txt`, `core/test_phase1b.py` |
| 248 | 2026-09-19 | **CI's first run found a test that had only ever passed because of an empty directory** | `core.test_phase8.DateHelperTests.test_the_module_is_gone` asserts `find_spec('payment.templatetags.uzb_dates')` is None, guarding §18 #25's deletion. `find_spec` returns None when a module is missing from a package **that exists**, and *raises* when the package itself is missing. `payment/templatetags/` was still on Kamronbek's disk — empty, untracked, left behind by that deletion — and Python reads a directory with no `__init__.py` as a namespace package, so the call returned None and the test passed. On a fresh clone the directory is simply not there and the same call raises. It would have failed the day it was written on any machine but the one that wrote it, and nothing had ever run the suite anywhere else. The assertion now treats both answers as gone, and the leftover directory is deleted so the two machines agree. **This is the whole argument for §17 #244** — the value of CI is not that it runs the tests, it is that it runs them somewhere else. Confirmed by running the full suite on Linux under Python 3.13 and PostgreSQL 16 before pushing again: 692 green | `core/test_phase8.py` |
| 245 | 2026-09-19 | **HTTP/2 is a `listen` parameter, not a directive — nginx 1.24 has no `http2 on;`** | `deploy/nginx/graphix.conf` was written in Phase 9 with `http2 on;`, which is correct and is nginx 1.25.1 or later. Ubuntu 24.04 ships **1.24.0**, which answers `unknown directive "http2"` and refuses to load the file. That is worse here than it sounds: this machine serves another site from the same nginx, so a file nginx cannot parse takes **both** down at the next restart, not just this one. Found the only way it could be — by putting the real file on the real server. Now `listen 443 ssl http2;`. Both blocks share `0.0.0.0:443` with the other site and nginx takes the protocol options of whichever block defines the socket first; `sites-enabled` is read alphabetically and `graphix.conf` sorts before `valleymade`, so ours wins and HTTP/2 is on for both. nginx says so with a `protocol options redefined` warning, which is expected rather than a fault. Verified `HTTP/2 200` from the server and `200` with a valid certificate from outside | `deploy/nginx/graphix.conf` |
| 246 | 2026-09-19 | **The certificate is issued with `certonly --webroot`, not the nginx plugin** | `certbot --nginx` needs a configuration nginx can already load, and `graphix.conf` cannot load until the certificate it names exists — a circle. So `graphix-acme.conf`, an http-only site, serves the challenge on port 80; certbot issues with `certonly --webroot`; `graphix.conf` names the certificate paths itself and replaces it. The TLS configuration therefore lives in the repository and is reviewable, rather than being whatever the plugin last wrote into the server's copy. Certificate issued 2026-09-19 for graphix.uz and www.graphix.uz, expiring 2026-12-18, renewal by certbot's own timer | `deploy/nginx/graphix-acme.conf`, `deploy/nginx/graphix.conf`, `deploy/RUNBOOK.md` |
...
| 44 | **The two SMS bodies are hardcoded Uzbek — no `gettext`, no Russian, no English** | 2026-09-20 | Found while extracting the exact text for Eskiz moderation. `user/otp.py` and `user/password_reset.py` build their message with an f-string, not `_()`, so a Russian- or English-speaking customer signing up is sent `GRAPHIX saytida ro'yxatdan o'tish uchun kodingiz: NNNNNN`. That is two user-facing strings bypassing §4's rule, and the only two left on the site. Uzbek-only may well be the right answer for an SMS — but it should be a decision, and it is currently an accident. **The moment to settle it is now**, before Eskiz moderates: each language is a separate body to submit, and submitting all of them together costs one round instead of two. Note the apostrophe is deliberately ASCII — see §17 #249 — and any translation has to keep that | 2026-09-20 | **Phase 1b / Phase 11.** |
...
| 45 | ~~**The checkout’s two radio groups are required choices with no mark**~~ **✅ Done 2026-09-22** — `role="radiogroup"` with `aria-required` and an accessible name of its own, closed with Phase 14 item 5 (§17 #264) | 2026-09-22 | Item 2 marked the six text fields and selects the server refuses to do without. `delivery_option` and `payment_method` are refused when absent too, but they are radio groups: the mark belongs on the group rather than on an option, which means a `role="radiogroup"` with an accessible name of its own — and the only heading available, `Yetkazib berish`, also heads the address fields below it, so a star on it would claim more than it means. Today neither group is usually empty: the delivery method is pre-checked on arrival, and the payment method is pre-checked whenever exactly one method is active. Item 5 takes that away — with cash, Click, Payme and Octo live, nothing is pre-checked and a customer can reach the button having chosen no payment method at all. **Phase 14 item 5.** |
...
| 46 | **Octo’s test flag rides on `DEBUG`** | 2026-09-23 | §17 #266 wired `is_test_mode=settings.DEBUG` into the Octo pay link, because the flag decides whether real money moves and it was previously defaulting to False on every developer machine. DEBUG is the right proxy for the two machines that exist today — a laptop (test) and production (live) — and it is the same value `TOLOV['OCTO_BANK']['TEST_MODE']` already uses, so the two halves agree. It stops being right the day there is a **staging server**: DEBUG off, but wanting test payments. §17 #267 then made it **one** line rather than three: the pay link and the configured-check both read `TOLOV['OCTO_BANK']['TEST_MODE']`, so an explicit `OCTO_TEST_MODE` in `.env` need only change that single value and everything follows. Not worth writing before the machine exists **Phase 11.** |
...
| 38 | **How is cash actually collected?** Cash is switched on as a payment option (§17 #253), but an ordinary Uzpost parcel has nobody to take money at handover — the courier does not collect on the shop’s behalf unless the UzPost partner programme is arranged (§19 Q24). So “cash” today means the customer pays the owner some other way, not cash on delivery. Kamronbek: *“just add it as an payment option … no need to futher explatination.”* Recorded rather than resolved; it is the owner’s to answer before a customer picks it | Phase 11 | ⏳ Open — raised 2026-09-21 |
| 37 | **The site is live, indexable, and has nothing in it.** `robots.txt` says `Allow: /` and the sitemap lists every page, so Google may index an empty shop — and a home page whose tagline promises “100+ dizayn” (§12 risk #14, Phase 11 item 2). A `Disallow: /` until launch keeps the first impression in search results from being an empty catalogue; leaving it open starts indexing earlier. Either is defensible; doing nothing by accident is not | Phase 11 | ✅ **Answered 2026-09-19** — §17 #247: `SITE_INDEXABLE` drives both `robots.txt` and a real `noindex`, default True, set False in the server’s `.env` until the catalogue exists. Verified live; Phase 11 deletes the setting from `.env` |
| 36 | **Is 57.128.240.51 the VPS bought on 2026-09-12, or an older machine?** The plan records a Warsaw VPS bought that day (§19 Q15, Q31) and an abandoned valleymade.uz (§17 #35) — but this address is serving valleymade.uz over HTTPS right now and **refuses Kamronbek's current SSH key for every user tried** (`root`, `ubuntu`, `debian`, `admin`, `deploy`, `valleymade`), which is what a machine set up long ago looks like, not one provisioned last week from this laptop. If there is a second, unused Warsaw instance, GRAPHIX belongs on that one. Answered from the OVH panel. **Nothing is installed until it is.** | Phase 1b | ✅ **Answered 2026-09-19 — yes, on the evidence.** Its specifications match §19 Q15 exactly (4 GB, 38 GB free, OVHcloud), Kamronbek authorised his key on it, and GRAPHIX has been deployed and is serving from it. The uptime said 13 days on 2026-09-18, which is a week before the plan records the purchase — Q15 was answered *“it is bought”* rather than *“bought today”*, so that is a recording detail rather than a second machine |
